const vm = new Vue({
    ...
    methods: {
      ...
      fetchComments(thread_id) {
        const url = "http://witty-kusu-4276.hippy.jp/2023Dev3Early/back/src/threadCommentDisplay.php";
      ...
      fetchThread_name(thread_id) {
        const url = "http://witty-kusu-4276.hippy.jp/2023Dev3Early/back/src/getThreadName.php";
        const timestamp = new Date().getTime(); // 毎回違うアドレスで検索するためのタイムスタンプ
        axios
          .get(`${url}?thread_id=${thread_id}&timestamp=${timestamp}`)
          .then((response) => {
